class Manage::Shop::ProductsController < Manage::PatternsController
  before_action :get_ticket_categories, only: [:new, :edit, :update, :show, :create]
  before_action :get_freight_templates, only: [:new, :edit, :update, :show, :create]
  ...
  def index
    ...
    if ((params["where"] || {})["title"] || {})["like"].present?
      ...
      query_option = {where: {active: true, shop_type: "Shop::Product"}, fields: [:title], limit: limit, offset: offset, page: params[:page] || 1}
  ...
  def get_freight_templates
    ...
    @templates = ::Shop::FreightTemplate.active
  end
end
