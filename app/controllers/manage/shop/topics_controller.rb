class Manage::Shop::TopicsController < Manage::PatternsController
  def index
    ...
    if ((params["where"] || {})["title"] || {})["like"].present?
      ...
      query_option = {where: {active: true, shop_type: "Shop::Topic"}, fields: [:title], limit: limit, offset: offset, page: params[:page] || 1}
      ...
      @records = @tasks = model.default(params).includes(:shop_task)
    end
  end
end
