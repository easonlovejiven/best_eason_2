class Manage::Shop::FundingOrdersController < Manage::PatternsController
  ...
  def export
    ...
    CoreLogger.info(controller: 'Manage::Shop::FundingOrders', action: 'export', format: params[:format], data: export.to_json, current_user: @current_user.try(:id))
    respond_to do |format|
      filename = "funding_orders-#{current_user.id}-#{export_params[:task_id] || "#{Time.parse(export_data[:paid_start_at]).strftime("%Y%m%d%H%M%S")}" }-#{Time.now.strftime("%Y%m%d%H%M%S")}"
      format.csv {
  ...
  def export_params
    {
      ...
      task_id: params["where"]["shop_funding_id"],
      task_type: 'Shop::Funding',
      paid_start_at: params["where"]["paid_at"][">="],
      paid_end_at: params["where"]["paid_at"]["<="],
      order_class: 'Shop::FundingOrder'
    }
  end
