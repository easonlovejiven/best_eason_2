class Shop::EventsController < Shop::ApplicationController
  ...
  def new
    ...
    if params[:create_event].present?
      @event = params[:create_event]
    else
  ...
  def edit
    @event = Shop::Event.active.find_by(id: params[:id])
  ...
  def update
    ...
    return redirect_to '/', alert: "没有权限" unless @event.user_id == @current_user.id && @current_user.verified
    shop_event_params[:star_list].map do |star|
    ...
    begin
      ActiveRecord::Base.transaction do
        @event.update!(shop_event_params) && @save_ret = true
      end
    rescue Exception => e
    ...
    if @save_ret == true
      CoreLogger.info(controller: "Shop::Events", action: 'update', event_id: @event.try(:id), current_user: @current_user.try(:id))
      if @event.shop_task.try(:published) == true
        render '/home/home/_step3', locals: {path: shop_event_path(@event), action: 'update'}
      else
        redirect_to preview_shop_event_path(@event)
      end
    else
  ...
  def show
    ...
    @task = Shop::Event.active.find_by(id: params[:id])
    ...
    @parti_users = Rails.cache.fetch("Shop_event_parti_users_#{params[:id]}", expires_in: 5.minutes) do
  ...
  def create
    ...
    begin
      @event = Shop::Event.new(shop_event_params)
      ActiveRecord::Base.transaction do
    ...
    if @save_ret == true
      CoreLogger.info(controller: "Shop::Events", action: 'create', event_id: @event.try(:id), current_user: @current_user.try(:id))
      redirect_to preview_shop_event_path(@event)
    else
      @event.errors.add(:title, "创建任务失败，请检查下活动内容哟～不要添加表情符号哦，O妹会崩溃的！") if @event.errors.full_messages.first.blank?
      render :new
  ...
  def more_users
    @task = Shop::Event.active.find_by(id: params[:id])
    @users = Rails.cache.fetch("get_shop_event_more_users_by_#{@task.id}", :expires_in => 5.minutes) do
      Shop::OrderItem.includes(:user).find_by_sql(
  ...
  def shop_event_params
    params.require(:shop_event)
      .permit(:id, :guide, :title, :shop_category, :description, :descripe_cover, :descripe2, :cover1, :cover2, :cover3, :key1, :key2, :key3, :start_at, :end_at, :sale_start_at, :sale_end_at, :address, :mobile, :user_id, :creator_id, :is_share, :free, :is_rush, :is_need_express, :freight_template_id,
