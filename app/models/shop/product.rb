class Shop::Product < ActiveRecord::Base
  ...
  has_many :order_items, as: :owhat_product, dependent: :destroy
  ...
  def manage_fields
    ...
    %w[ id is_rush guide shop_category title funding_target description descripe_cover descripe2 key1 key2 key3 cover1 cover2 cover3 start_at end_at sale_start_at sale_end_at address mobile star_list user_id creator_id is_share free is_need_express freight_template_id ] << {star_list: [], ticket_types_attributes: ticket_types_attributes, ext_infos_attributes: ext_infos_attributes}
  end
  ...
  def order_items_size
    Rails.cache.fetch("order_items_size_by_task_id_#{self.id}_and_task_type_shop_products", expires_in: 5.minutes) do
      Shop::OrderItem.where(owhat_product_id:self.id, owhat_product_type: self.class.to_s).group(:status).count
    end
  end

  def sum_fee
    Rails.cache.fetch("sum_fee_by_task_id_#{self.id}_and_task_type_shop_products", expires_in: 5.minutes) do
      self.order_items.where(status: 2).map(&:payment).sum.to_f.round(2)
    end
  end

  def unwithdraw_order_fee
    Rails.cache.fetch("unwithdraw_order_fee_by_task_id_#{self.id}_and_task_type_shop_products", expires_in: 5.minutes) do
      self.order_items.where(status: 2).map(&:payment).sum.to_f.round(2) - Core::WithdrawOrder.where(status: 3, task_id: self.id, task_type: "Shop::Product").map(&:amount).sum.to_f.round(2)
    end
  end
end
